...
public class RevisionAnalyser {
	...
	void analyse(RevisionDto revisionDto) {
		...
		LOG.info("Entity {}, context {}, changed: {}", revisionDto.getEntityId(), revisionDto.getContext(),
				result.stream().map(RevisionComparisonDto::toString).collect(Collectors.joining()));
	}
